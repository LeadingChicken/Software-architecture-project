...
const GameCount = () => {
  const [games, setBrands] = useState([]);
  ...
  useEffect(() => {
    const getBrands = async () => {
      try {
        const gameData = await fetchBrands();
        setBrands(gameData); // Cập nhật state với danh sách người dùng
      } catch (error) {
  ...
  return (
    <div className={`text-center ${styles.container}`}>
      ...
      <p className="mb-3 display-6"> {games.length}</p>{" "}
      <div className="d-flex justify-content-center display-7">
        <div>
          {/* Render số lượng người dùng */}
          <ul className="mb-2">
            {games.map((game) => (
              <li key={game.id}>{game.gameName}</li> // Hiển thị thông tin người dùng
            ))}
          </ul>
        </div>
      </div>
